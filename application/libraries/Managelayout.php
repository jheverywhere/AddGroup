...
class Managelayout extends CI_Controller
{
	...
	function front($config = array(), $device_view_type = '')
	{
		...
		if ($CI->input->is_ajax_request() === false) {
			...
			$menu = $CI->Menu_model->get_all_menu($device_view_type);
			// debug_var($menu);
			$data['menu'] = array();
			// if ($CI->member->is_admin()) {
			// 	$data['menu'] = $menu;
			// } else {
				$allow_menu_list = array();

				if ($CI->member->item('mem_level') >= '10') {
					$allow_menu_list[] = "공지사항";
				}
				if ($CI->member->item('mem_level') >= '20') {
					$allow_menu_list[] = "공원관리";
					$allow_menu_list[] = "일정관리";
					$allow_menu_list[] = "고객등록";
					$allow_menu_list[] = "진행조회";
					$allow_menu_list[] = "정산내역";
					$allow_menu_list[] = "시설현황발송내역";
					$allow_menu_list[] = "고객문자발송내역";
				}
				if ($CI->member->item('mem_level') >= '40') {
				}
				if ($CI->member->item('mem_level') >= '50') {
					$allow_menu_list[] = "공원추가";
					$allow_menu_list[] = "직원관리";
					$allow_menu_list[] = "직원추가";
				}
				if ($CI->member->item('mem_level') >= '10') {
					$allow_menu_list[] = "엑셀다운로드";
				}

				$CI->load->model("Mp_Contract_model");
				$summary = $CI->Mp_Contract_model->get_summary();
				// debug_var($summary);

				foreach ($menu[0] as $key => $menu_row) {
					if (in_array(element('men_name', $menu_row), $allow_menu_list)) {
						if (element('men_name', $menu_row) == '진행조회') {
							$menu_row['men_name'] .= " <span class=\"badge\">".get_count_str(element('total', $summary))."</span>";
						}
						$data['menu'][0][$key] = $menu_row;
					}
				}
				...
				foreach ($menu as $key => $menu_row) {
					...
					foreach ($menu_row as $sub_men_id => $sub_menu_row) {
						if (element('men_name', $sub_menu_row) == '진행중') {
							$menu_row[$sub_men_id]['men_name'] .= " <span class=\"badge\">".get_count_str(element('continue', $summary))."</span>";
						} else if (element('men_name', $sub_menu_row) == '계약완료') {
							$menu_row[$sub_men_id]['men_name'] .= " <span class=\"badge\">".get_count_str(element('hold', $summary))."</span>";
						} else if (element('men_name', $sub_menu_row) == '잔금완료') {
							$menu_row[$sub_men_id]['men_name'] .= " <span class=\"badge\">".get_count_str(element('complete', $summary))."</span>";
						} else if (element('men_name', $sub_menu_row) == '진행불발') {
							$menu_row[$sub_men_id]['men_name'] .= " <span class=\"badge\">".get_count_str(element('misfire', $summary))."</span>";
						}
					}
					$data['menu'][$key] = $menu_row;
				}
			// }
