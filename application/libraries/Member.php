...
class Member extends CI_Controller
{
	...
	function __construct()
	{
        ...
        $this->CI->load->model( array('Member_userid_model'));
	 ...
	 * 로그인 (token)
	 */
	public function login_token($tk)
	{
        $token_member = $this->CI->Member_userid_model->get_one('', '*', "mem_token='{$tk}'");
        return $token_member;
	}
	 ...
	 * 감사 기록을 남깁니다
	 */
	public function update_audit_log($type, $reason, $mem_id= 0, $userid = '')
	{
		$mem_id = (int) $mem_id ? (int) $mem_id : $this->is_member();
		$userid = $userid ? $userid : $this->item('mem_userid');
		$reason = isset($reason) ? $reason : '';
		$referer = $this->CI->input->get_post('url', null, '');
		$auditlog = array(
			'mad_type' => $type,
			'mem_id' => $mem_id,
			'mad_userid' => $userid,
			'mad_datetime' => cdate('Y-m-d H:i:s'),
			'mad_ip' => $this->CI->input->ip_address(),
			'mad_reason' => $reason,
			'mad_useragent' => $this->CI->agent->agent_string(),
			'mad_url' => current_full_url(),
			'mad_referer' => $referer,
		);
		$this->CI->load->model('Member_audit_log_model');
		$this->CI->Member_audit_log_model->insert($auditlog);
