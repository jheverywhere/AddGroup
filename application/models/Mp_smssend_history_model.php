...
/**
 * Mp_smssend_history_model class
 */

class Mp_smssend_history_model extends CB_Model
{

	/**
	 * 테이블명
	 */
	public $_table = 'mp_smssend_history';
	...
	public function get_sms_list($offset=0, $limit='', $where='', $like='', $where_in='')
	{
		...
		$this->db->select("
			...
			cb_mp_smssend_history.*
		");
		$this->db->from("cb_mp_smssend_history");
		// $this->db->join("cb_mp_sms_recv", "cb_mp_sms_recv.sms_no = cb_mp_smssend_history.sms_no", "LEFT");
		$this->db->join("cb_mp_contract", "cb_mp_contract.ctr_no = cb_mp_smssend_history.sms_ctr_no", "INNER");
		$this->db->join("cb_mp_sms_recv", "cb_mp_sms_recv.sms_no = cb_mp_smssend_history.sms_no", "INNER");
		...
		$this->db->group_by("cb_mp_smssend_history.sms_no");
	...
	public function get_sms_list_by_ctr_no($ctr_no, $where='', $like='')
	{
		...
		$this->db->where('cb_mp_smssend_history.sms_ctr_no', $ctr_no);
		if ($where) {
			$this->db->where($where);
		}
		if ($like) {
