 ...
 * 핸드폰 번호 얻기
 */
if ( ! function_exists('get_phone_by_masked')) {
	function get_phone_by_masked($phone, $hyphen=1)
	{
		$phone = get_phone($phone, $hyphen);
		return preg_replace('/([0-9]+)-([0-9]+)-([0-9]{4})/', '${1}-****-$3', $phone); // 가운데 4자리
		// return $phone ? substr($phone, 0, 4)."***".substr($phone, 7) : ""; // 가운데 3자리만
	}
}
...
if ( ! function_exists('get_selected')) {
	function get_selected($val1 = '', $val2 = '')
	{
		if (is_array($val2)) {
			return in_array($val1, $val2) ? " selected" : "";
		} else {
			return (trim($val1) == trim($val2)) ? " selected" : "";
		}
	}
}
 ...
 * checked 여부 확인
 */
if ( ! function_exists('get_checked')) {
	function get_checked($val1 = '', $val2 = '')
	{
		if (is_array($val1)) {
			return in_array($val2, $val1) ? " checked" : "";
		} else if (is_array($val2)) {
			return in_array($val1, $val2) ? " checked" : "";
		} else {
			return (trim($val1) == trim($val2)) ? " checked" : "";
		}
	}
}
...
if ( ! function_exists('combine_address')) {
    function combine_address($addr1='', $addr2='', $addr3='', $addr4='')
    {
		$list = array();
		if ($addr1) $list[] = $addr1;
		if ($addr2) $list[] = $addr2;
...
if ( ! function_exists('display_price')) {
    function display_price($price, $unit='')
    {
		switch ($unit) {
			case '십':   $price /= 10; break;
			case '백':   $price /= 100; break;
			case '천':   $price /= 1000; break;
			case '만':   $price /= 10000; break;
			case '십만': $price /= 100000; break;
			case '백만': $price /= 1000000; break;
			case '천만': $price /= 10000000; break;
			case '억':   $price /= 100000000; break;
		}
		if (0 <= $price) {
			return number_format($price).$unit."원";
		} else {
			return "<span style=\"color: #FF0000;\">".number_format($price).$unit."원"."</span>";
		}
    }
}
...
if ( ! function_exists('get_manwon')) {
    function get_manwon($price)
    {
		return $price / 10000;
    }
}
...
if ( ! function_exists('get_count_str')) {
    function get_count_str($num)
    {
        return number_format($num)."건";
    }
}
...
if ( ! function_exists('get_phone_link')) {
	function get_phone_link($phone, $class="btn btn-info btn-xs")
	{
		if ($phone) {
			$result = array();
			$phone_list = explode(";", $phone);
			foreach ($phone_list as $phone_number) {
				$formatted_phone_number = get_phone($phone_number);
				$result[] = "<a href=\"tel:{$formatted_phone_number}\" class=\"{$class}\"><span class=\"glyphicon glyphicon-earphone\" aria-hidden=\"true\"></span>&nbsp;{$formatted_phone_number}</a>";
			}
...
if ( ! function_exists('get_cust_phone_link')) {
	function get_cust_phone_link($phone, $name='', $class="btn btn-info btn-xs")
	{
		if ($phone) {
				$result = array();
				$phone_list = explode(";", $phone);
				foreach ($phone_list as $phone_number) {
						$formatted_phone_number = get_phone($phone_number);
						$result[] = "<button type=\"button\" class=\"{$class} btn_phone_link\" data-toggle=\"popover\" data-content=\"
						<a href='tel:{$formatted_phone_number}' class='btn btn-success'><span class='glyphicon glyphicon-earphone' aria-hidden='true'></span></a>
						<button type='button' class='btn btn-success' onclick='go_smssend(this)' data-nm='{$name}' value='{$formatted_phone_number}'><span class='glyphicon glyphicon-comment' aria-hidden='true'></span></button>
						\"><span class='glyphicon glyphicon-link' aria-hidden='true'></span>&nbsp;{$formatted_phone_number}</button>".PHP_EOL;
				}
				return implode("", $result);
		} else {
				return "-";
...
if ( ! function_exists('get_week_string')) {
    function get_week_string($date_string)
    {
		$week = array("일", "월", "화", "수", "목", "금", "토");
		return $week[date('w', strtotime($date_string))];
    }
}

//------------------------------------------------------------------------------

if ( ! function_exists('get_ymdhi')) {
    function get_ymdhi($date_string)
    {
		if ($date_string) {
...
if ( ! function_exists('get_ymdhi_string')) {
    function get_ymdhi_string($date_string)
    {
		if ($date_string) {
			return date("Y-<\b\\r>m-d<\b\\r>H:i", strtotime($date_string));
		} else {
			return "-";
		}
    }
...
if ( ! function_exists('get_ymd_string')) {
    function get_ymd_string($date_string)
    {
		if ($date_string) {
        	return date("Y-<\b\\r>m-d", strtotime($date_string));
		} else {
			return "-";
		}
    }
...
if ( ! function_exists('get_history_type_list')) {
    function get_history_type_list()
    {
        return array(
			"방문답사" => array('color' => "#ff6b6b"),
			"시간미정" => array('color' => "#cf9f6b"),
			"재방답사" => array('color' => "#ff9f43"),
			"타당2차" => array('color' => "#feca57"),
			"개장일정" => array('color' => "#10ac84"),
			"안치일정" => array('color' => "#54a0ff"),
			"잔금기일" => array('color' => "#2e86de"),
			"기타" => array('color' => "#9b59b6"),
		);
    }
}
...
if ( ! function_exists('get_park_prod_cd')) {
    function get_park_prod_cd($park_type_cd, $park_no, $park_prod_seq, $park_opt1, $park_opt2, $park_anchisu)
    {
		$park_prod_cd =
			$park_type_cd.
			str_pad(base_convert($park_no, 10, 36), 2, '0', STR_PAD_LEFT).
			sprintf("%02d", $park_prod_seq).
			str_pad($park_opt1, 2, '0', STR_PAD_LEFT).
			str_pad($park_opt2, 2, '0', STR_PAD_LEFT).
			sprintf("%02d", $park_anchisu);
		return $park_prod_cd;
    }
}
...
if ( ! function_exists('get_park_type_name')) {
    function get_park_type_name($park_type_cd)
    {
		$park_type_list = get_park_type_list();
		return element($park_type_cd, $park_type_list);
    }
}

//------------------------------------------------------------------------------

if ( ! function_exists('get_park_type_list')) {
    function get_park_type_list()
    {
		return array(
			"NB" => "수목장",
			"CH" => "납골당",
...
if ( ! function_exists('get_highlight_keyword')) {
    function get_highlight_keyword($content, $keyword, $color="#FF0000")
    {
		return str_replace($keyword, "<strong style=\"color: {$color}\">{$keyword}</strong>", $content);
    }
}

//------------------------------------------------------------------------------

if ( ! function_exists('get_readonly_for_level_20')) {
    function get_readonly_for_level_20()
    {
		$CI =& get_instance();
		return $CI->member->item('mem_level') <= 20 ? ' readonly' : '';
    }
}

//------------------------------------------------------------------------------

if ( ! function_exists('get_disabled_for_level_20')) {
    function get_disabled_for_level_20()
    {
		$CI =& get_instance();
		return $CI->member->item('mem_level') <= 20 ? ' disabled' : '';
    }
}
