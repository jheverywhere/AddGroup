...
class Deposit extends CB_Controller
{
    ...
    private function login_check($tk)
    {
        $headers = $this->input->request_headers();
        if ($tk != "" && array_key_exists("X-Requested-With", $headers) && $headers['X-Requested-With'] == 'com.webist.vtapp') {
            $CI =& get_instance();
            if (!$CI->member->is_member()) {
                $use_login_account = $this->cbconfig->item('use_login_account');
                $token_member = $CI->member->login_token($tk);
                if ($token_member) {
                    if ($use_login_account === 'both') {
                        $userinfo = $this->Member_model->get_by_both($token_member['mem_userid'], 'mem_id, mem_userid');
                    } elseif ($use_login_account === 'email') {
                        $userinfo = $this->Member_model->get_by_email($token_member['mem_userid'], 'mem_id, mem_userid');
                    } else {
                        $userinfo = $this->Member_model->get_by_userid($token_member['mem_userid'], 'mem_id, mem_userid');
                    }
                    $this->member->update_login_log(element('mem_id', $userinfo), $token_member['mem_userid'], 1, '로그인 성공');
                    $this->session->set_userdata(
                        'mem_id',
