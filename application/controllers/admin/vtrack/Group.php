...
class Group extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'vtrack/group';

	/**
	 * 모델을 로딩합니다
	 */
	protected $models = array('Vehicle_group');

	/**
	 * 이 컨트롤러의 메인 모델 이름입니다
	 */
	protected $modelname = 'Vehicle_group_model';
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_vehicle_vehiclegroup_index';
		...
		$view['view']['sort'] = array(
            'vgr_id' => $param->sort('vgr_id', 'asc'),
            'vgr_name' => $param->sort('vgr_name', 'asc'),
		);
        $findex = $this->input->get('findex', null, 'vgr_id');
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('vgr_name', 'vgr_desc'); // 검색이 가능한 필드
		$this->{$this->modelname}->search_field_equal = array(); // 검색중 like 가 아닌 = 검색을 하는 필드
		$this->{$this->modelname}->allow_order_field = array('vgr_name'); // 정렬이 가능한 필드
		 ...
		 */
		$search_option = array('vgr_name' => '그룹명');
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_vehicle_vehiclegroup_write';
		$this->load->event($eventname);
        ...
        if ($pid) { // 수정인 경우
			...
			$config[] = array(
				'field' => 'vgr_id',
				'label' => '그룹아이디',
				'rules' => 'trim|required|alpha_dash|min_length[36]|max_length[36]|is_unique[vt_vehicle.vgr_id.' . element('vgr_id', $getdata) . ']',
			);
		}
		...
		if ($this->form_validation->run() === false) {
			...
			$updatedata = array(
				...
				'vgr_parent_id' => $this->input->post('vgr_parent_id', null, ''),
				'vgr_name'      => $this->input->post('vgr_name', null, ''),
				'vgr_desc'      => $this->input->post('vgr_desc', null, ''),
			);
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_vehicle_vehiclegroup_listupdate';
		$this->load->event($eventname);
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			...
			$vgr_parent_id = $this->input->post('vgr_parent_id');
			$vgr_name      = $this->input->post('vgr_name');
			$vgr_desc      = $this->input->post('vgr_desc');

			foreach ($this->input->post('chk') as $primary_key) {
				if ($primary_key) {
                    $updatedata = array(
                        'vgr_parent_id' => element($primary_key, $vgr_parent_id),
                        'vgr_name'      => element($primary_key, $vgr_name),
                        'vgr_desc'      => element($primary_key, $vgr_desc),
                    );
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_vehicle_vehiclegroup_listdelete';
		$this->load->event($eventname);
