...
/**
 * 관리자>게시판설정>파일다운로드 controller 입니다.
 */
class Filedownload extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'board/filedownload';

	/**
	 * 모델을 로딩합니다
	 */
	protected $models = array('Board', 'Post_file_download_log');

	/**
	 * 이 컨트롤러의 메인 모델 이름입니다
	 */
	protected $modelname = 'Post_file_download_log_model';
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_filedownload_index';
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('pfd_id', 'pfi_id', 'post.post_id', 'post.mem_id', 'post_file_download_log.mem_id', 'pfd_datetime', 'pfd_ip'); // 검색이 가능한 필드
		$this->{$this->modelname}->search_field_equal = array('pfd_id', 'pfi_id', 'post.post_id', 'post.mem_id', 'post_file_download_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
		$this->{$this->modelname}->allow_order_field = array('pfd_id'); // 정렬이 가능한 필드
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$select = 'mem_id, mem_userid, mem_nickname, mem_username, mem_icon';
				...
				$result['list'][$key]['download_link'] = admin_url('board/fileupload/download/' . element('pfi_id', $val));
				...
				if (element('pfd_useragent', $val)) {
					$userAgent = get_useragent_info(element('pfd_useragent', $val));
		 ...
		 */
		$search_option = array('pfd_datetime' => '날짜', 'pfd_ip' => 'IP');
	...
	public function graph($export = '')
	{
		...
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_filedownload_graph';
		$this->load->event($eventname);
		...
		$result = $this->{$this->modelname}->get_post_file_download_count($datetype, $start_date, $end_date, $brd_id, $orderby);
		$sum_count = 0;
		$arr = array();
		$max = 0;
		...
		if ($export === 'excel') {
			...
			header('Content-Disposition: attachment; filename=파일다운로드_' . cdate('Y_m_d') . '.xls');
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_filedownload_listdelete';
		$this->load->event($eventname);
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_filedownload_cleanlog';
		$this->load->event($eventname);
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				...
				$view['view']['alert_message'] = '총 ' . number_format($this->input->post('log_count')) . ' 건의 ' . $this->input->post('day') . '일 이상된 파일다운로드로그가 모두 삭제되었습니다';
			} else {
				...
				if ($log_count > 0) {
					$view['view']['msg'] = '총 ' . number_format($log_count) . ' 건의 ' . $this->input->post('day') . '일 이상된 파일다운로드로그가 발견되었습니다. 이를 모두 삭제하시겠습니까?';
				} else {
					$view['view']['alert_message'] = $this->input->post('day') . '일 이상된 파일다운로드로그가 발견되지 않았습니다';
				}
			}
		}
