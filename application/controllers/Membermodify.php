		...
		if ($form_validation === false OR $file_error !== '' OR $file_error2 !== '') {
			...
			if ($form && is_array($form)) {
				foreach ($form as $key => $value) {
                    ...
                    if (element('field_name', $value) === 'mem_biz_address1'
                        OR element('field_name', $value) === 'mem_biz_address2'
                        OR element('field_name', $value) === 'mem_biz_address3'
                        OR element('field_name', $value) === 'mem_biz_address4') {
                            continue;
                    } elseif (element('field_name', $value) === 'mem_biz_zipcode') {
						$html_content[$k]['input'] .= '
							<label for="mem_biz_zipcode">우편번호</label>
							<label>
								<input type="text" name="mem_biz_zipcode" value="' . set_value('mem_biz_zipcode', $this->member->item('mem_biz_zipcode')) . '" id="mem_biz_zipcode" class="form-control input" size="7" maxlength="7" ' . $required . '/>
							</label>
							<label>
								<button type="button" class="btn btn-black btn-sm" style="margin-top:0px;" onclick="win_zip(\'fregisterform\', \'mem_biz_zipcode\', \'mem_biz_address1\', \'mem_biz_address2\', \'mem_biz_address3\', \'mem_biz_address4\');">주소 검색</button>
							</label>
							<div class="addr-line mt10">
								<label for="mem_biz_address1">기본주소</label>
								<input type="text" name="mem_biz_address1" value="' . set_value('mem_biz_address1', $this->member->item('mem_biz_address1')) . '" id="mem_biz_address1" class="form-control input" placeholder="기본주소" ' . $required . ' />
							</div>
							<div class="addr-line mt10">
								<label for="mem_biz_address2">상세주소</label>
								<input type="text" name="mem_biz_address2" value="' . set_value('mem_biz_address2', $this->member->item('mem_biz_address2')) . '" id="mem_biz_address2" class="form-control input" placeholder="상세주소" ' . $required . ' />
							</div>
							<div class="addr-line mt10">
								<label for="mem_biz_address3">참고항목</label>
								<input type="text" name="mem_biz_address3" value="' . set_value('mem_biz_address3', $this->member->item('mem_biz_address3')) . '" id="mem_biz_address3" class="form-control input" readonly="readonly" placeholder="참고항목" />
							</div>
							<input type="hidden" name="mem_biz_address4" value="' . set_value('mem_biz_address4', $this->member->item('mem_biz_address4')) . '" />
						';
                    } elseif (element('field_name', $value) === 'mem_biz_attach') {
						$html_content[$k]['input'] .= '
                            <input type="file" name="mem_biz_attach" id="mem_biz_attach" />
                            <p class="help-block">gif, jpg, png 파일 업로드가 가능합니다</p>
                        ';
                    } elseif (element('field_name', $value) === 'mem_nickname') {
                        $html_content[$k]['input'] .= '
                            <input type="hidden" name="mem_nickname" value="'.set_value(element('field_name', $value), $item).'" id="mem_nickname" />
                        ';
					//field_type : text, url, email, phone, textarea, radio, select, checkbox, date
                    } elseif (element('field_type', $value) === 'text'
			...
			if ($can_update_nickname
				...
				&& $this->member->item('mem_nickname') !== $this->input->post('mem_username', null, '')) {
                $updatedata['mem_nickname'] = $this->input->post('mem_username', null, '');
				...
				$nickinsert = array(
                    ...
                    'mni_nickname' => $this->input->post('mem_username', null, ''),
